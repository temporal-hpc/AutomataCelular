...
// rules
#define EL 2  // Enviroment
#define EU 3
#define FL 3  // Fertility
#define FU 3
...
__global__ void kernel_update_ghosts(const unsigned int n, const unsigned long msize, MTYPE *dmat1, MTYPE *dmat2){
    ...
    if(tid >= n){return;}
    // left
    dmat2[(tid+1)*(n+2) + 0]        = dmat1[(tid+1)*(n+2) + n];
    //bottom
    dmat2[(n+1)*(n+2) + (tid+1)]    = dmat1[tid + 1 + (n+2)];
    //top
    dmat2[tid + 1]                  = dmat1[(n+1)*(n+2) + (tid+1) - (n+2)];
    //right
    dmat2[(tid+1)*(n+2) + (n+1)]    = dmat1[(tid+1)*(n+2) + 1];
...
__device__ __inline__ void kernel_update_ghosts_cooperative_groups(const unsigned int n, const unsigned long msize, MTYPE *dmat1, MTYPE *dmat2){
    // update ghosts cells
    // this kernel uses a linear grid of size n
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if(tid >= n){return;}
...
template<typename Lambda>
 __global__ void k_cg(const unsigned int n, const unsigned long msize, DTYPE *data, MTYPE *dmat1, MTYPE *dmat2, int bal, unsigned int aux1, unsigned int aux2, unsigned int aux3, unsigned int REPEATS){
	printf("Esti");
 }


template<typename Lambda>
 __global__ void kernel_test_cooperative_groups(const unsigned int n, const unsigned long msize, DTYPE *data, MTYPE *dmat1, MTYPE *dmat2, Lambda map, unsigned int aux1, unsigned int aux2, unsigned int aux3, unsigned int REPEATS){
	...
	p = map(n, msize, aux1, aux2, aux3);
        load_cache(cache, dmat2, n, threadIdx, p);
        if(/*p.x <= p.y && */p.y < n){
            //work_cache(data, dmat1, dmat2, cache, threadIdx, p, n);
	    work_nocache(data, dmat2, dmat1, p, n);
        }
	cooperative_groups::sync(cooperative_groups::this_grid());
    }
}
