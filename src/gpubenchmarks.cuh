...
double bbox_cg(const unsigned int n, const unsigned int REPEATS, double DENSITY){
    #ifdef DEBUG
        printf("[Bounding Box]\n");
    #endif
    DTYPE *hdata, *ddata;
    ...
    // benchmark
    double time = benchmark_map_cooperative_groups(REPEATS, block, grid, n, msize, trisize, ddata, dmat1, dmat2, map, 0, 0, 0);
    // check result
    double check = (float)verify_result(n, msize, hdata, ddata, hmat, dmat2);
	cudaFree(ddata);
	cudaFree(dmat1);
