...
template<typename Lambda>
double benchmark_map_cooperative_groups(const int REPEATS, dim3 block, dim3 grid, unsigned int n,
        unsigned long msize, unsigned int trisize, DTYPE *ddata, MTYPE *dmat1,
        MTYPE *dmat2, Lambda map, unsigned int aux1, unsigned int aux2, unsigned int aux3){
	cudaEvent_t start, stop;
	cudaEventCreate(&start);
    ...
    params[7] = (void *)&aux2;
    params[8] = (void *)&aux3;
    params[9] = (void *)&REPEATS;

  
	printf("block= %i x %i x %i    grid = %i x %i x %i\n", block.x, block.y, block.z, grid.x, grid.y, grid.z);
    cudaLaunchCooperativeKernel((void *) kernel_test_cooperative_groups<Lambda>, grid, block, params, 0, NULL);
    //cudaLaunchCooperativeKernel((void *) k_cg, grid, block, params, 0, NULL);
    cudaDeviceSynchronize();
    last_cuda_error("error CG");
    printf("paso\n");
/* 
        kernel_update_ghosts<<< (n+BSIZE1D-1)/BSIZE1D, BSIZE1D>>>(n, msize, dmat1, dmat1);
        cudaDeviceSynchronize();
